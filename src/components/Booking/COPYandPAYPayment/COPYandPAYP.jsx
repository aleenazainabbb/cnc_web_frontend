...
const PaymentPage = () => {
  ...
  const validateForm = () => {
    if (!paymentForm.bookingId.trim()) {
      setError('Booking ID is required');
      return false;
    }
    const billing = paymentForm.billingDetails;
    const required = ['street', 'city', 'country', 'postcode'];
    const missing = required.filter(f => !billing[f]?.trim());
    if (missing.length) {
  ...
  const handlePrepareCheckout = async (e) => {
    ...
    setError(null);
    try {
      const response = await prepareCheckout(paymentForm);
      if (response.success) {
        setCheckoutData(response.data);
        loadPaymentWidget(response.data.checkoutId);
      } else {
          ...
          {bookingDetails && (
            <div className={styles.summary}>
              ...
              <div className={styles.summaryRow}>
                <span>Booking ID:</span>
                <span>{bookingDetails.bookingId || `B-${String(bookingDetails.id).padStart(3, '0')}`}</span>
              </div>
              <div className={styles.summaryRow}>
                <span>Service:</span>
                <span>{bookingDetails.serviceName || 'Cleaning Service'}</span>
              </div>
              <div className={styles.summaryRow}>
                <span>Amount:</span>
                <span>AED {parseFloat(bookingDetails.cncChargesInclVat || bookingDetails.totalPrice || 0).toFixed(2)}</span>
              </div>
            </div>
          )}
            ...
            {!checkoutData ? (
              ...
              <div className={styles.widgetSection}>
                <div className={styles.widgetHeader}>
                  ...
                  <button onClick={handleReset}>← Change Billing Info</button>
                </div>
                <div className={styles.widgetBox}>
                  ...
                  <form className="paymentWidgets" data-brands="VISA MASTER MADA"></form>
                </div>
              </div>
            )}
          </div>
